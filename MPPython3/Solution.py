...
class Solution:
    ...
    def improve_MST(self):
        ...
        while len(unhappy) and (len(unhappy) < last_unhappy_cnt or last_useful): # exit loop if we got everyone or we cant get anymore
            ...
            scored_paths = [ (p, self.score_current_path(p)) for p in new_paths.values() ]
            sorted_new_paths = sorted(scored_paths, reverse=True, key=p[0]) # O(n^2)
    ...
    def output_paths(self):
        ...
        return (self.improve_MST(), bandwidths, priorities)
